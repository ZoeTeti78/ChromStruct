...
warranty="Disclaimer: This program is distributed in the hope that it will be useful,\n"\
          +"but WITHOUT ANY WARRANTY; without even the implied warranty of\n"\
...
conditions="This program is free software: you can redistribute it and/or modify\n"\
            +"it under the terms of the GNU General Public License as published by\n"\
            +"the Free Software Foundation, version 3 of the License\n"\
            +"<http://www.gnu.org/licenses/>.\n\n (C) 2016-2017, E. Salerno and C. Caudai\n\n"\
            +"{emanuele.salerno,claudia.caudai}@isti.cnr.it"
...
# IEEE/ACM TCBB, submitted, 2017
#

#
# CODE VERSION
version="4.2_GUI (Nov 20, 2017)"
...
#
class bead:
    def __init__(self,endp1=Vector3(0.,0.,0.),centrd=Vector3(0.,0.,0.),endp2=Vector3(0.,0.,0.),ext=15.):
        self.centrd=centrd
        self.endp1=endp1
        self.endp2=endp2
        self.ext=ext
...
def block(level,contacts,bigblocks,window=3,minsize=4,span=7):
    if level==0:
        blocks=[]
        bounds=[0]
        sizes=[]
...
def centrsize(n):
    G=crate*RIS
    bead2=bead(Vector3(0.,0.,0.),Vector3(G/2.,0.,0.),Vector3(G,0.,0.),ext=G/2.)
    return bead2
...
def inizchain(hm,centromere,telomeres,NMAX=0):
...
# FUNCTION compose
#
# Iterative procedure to reconstruct the full-resolution chain from the lower-resolution versions
...
#
# Iterative procedure to reconstruct the full-resolution chain
def chromstruct(filen,contacts,chaininf,bigblocks,level=0):
...
def mainchrom(filen,ParList):
    ...
    string=       "\n\n========================================\nCHROMSTRUCT ver "+version\
                  +"\nCopyright (C) 2016-2017 Emanuele Salerno, Claudia Caudai\n\n"
    print string
    ...
    conf=inizchain(hm,centromere,telomeres,NMAX)                          # Set initial guess
    ...
    chromstruct(filen,hm,conf,bigblocks)                             # Call iteration
...
class Chromstruct(Frame):
    ...
    def initUI(self):
        ...
        global fn, DIA, RIS, NB, crate, NMAX, extrate, span, window,\
               ...
               RANDPLA, RANDDIE, decrtemp,path,intplot, format1, format2

        self.parent.title("ChromStruct 4.2")
        ...
        fn=StringVar()
        filen = Button(self, text="INPUT DATA",command=self.openfile)
        ...
        Label(self,text="Output format").grid(row=14,column=0,columnspan=2)
        format1=IntVar()
        Checkbutton(self,text="txt",variable=format1).grid(row=14,column=0, columnspan=3)
        format2=IntVar()
        Checkbutton(self,text="PDB",variable=format2).grid(row=14,column=0, columnspan=4)
        
##        quitButton=Button(self,text="Quit",command=self.quit)
##        quitButton.grid(row=14,column=0)

        testo=Text(self,width=91,height=1,bg='#40E0D0')
        testo.grid(row=16,column=0,columnspan=3)
        hyperlink=HyperlinkManager(testo)
        def click():
            tkMessageBox.showinfo("Conditions of Use",warranty+conditions)
        testo.insert(INSERT,"(C) 2016-2017, Emanuele Salerno and Claudia Caudai. License GNU-GPL v.3. ")
    ...
    def openfile(self,*args):
        global fn, path
        fn.set(tkFileDialog.askopenfilename())
