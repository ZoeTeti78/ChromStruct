...
## Required minimum number of blocks (resume if needed)
...
def chromstruct(filen,contacts,chaininf,level=0):
    ...
    for i in range(n):             # Scan the subchains (intrinsically parallelizable)
        ...
        if display[0] == 'y':
            ...
            # Plot accepted vs proposed updates in the current block
        ...
        savepar(filen,Phiseries, level, i)                  # Save the features of the annealing schedule
        saveconf(filen,C,level,i)                           # Save current subchain configuration
    ...
    if n>1:                                           # Increase level if appropriate
        ...
        chromstruct(filen,contacts,newchain,level)          # call procedure at the successive level
    else:
        if level > 0:
            Clast=compose(filen,C,level)                    # recompose full-resolution chain
        else:
            Clast=C
...
def mainchrom(filen,ParList):
    global diagneg, datafact,energy_scale, energy_exp, span, window, minsize, regulenergy,\
           ...
           StopTolerance, RANDPLA, RANDDIE, decrtemp, DIA, RIS, NB, NC, LMAX, LMIN, NMAX, extrate,\
           timemark, outdata, display
    ...
    #
    # Fitness part
    diagneg=ParList[8]             # # neglected diagonals in contact matrix (in populate, to select the relevant pairs for data fit)
    datafact=ParList[9]           # Fraction of the pairs per block used to build the penalty function (real, in populate)

    # Constraint part
    energy_scale=ParList[10]    # Tunes the extent of the moderate penalty range around the threshold distance 
                               # (floating; physical dimensions 1/length; measurement unit: reciprocal of the units used for DIA)
    energy_exp=ParList[11]               # Tunes the slopes of the penalty in the transitions between the moderate range
                               # and the external intervals (odd integer, dimensionless)

    #
    # TAD EXTRACTION (FUNCTION block)
    span=ParList[5]                # SIZE OF THE MOVING TRIANGLE
    window=ParList[6]              # MOVING AVERAGE WINDOW APERTURE
    minsize=ParList[7]             # MINIMUM REQUIRED DIAGONAL BLOCK SIZE


    #
    # ANNEALING: DETERMINATION OF THE REGULARIZATION PARAMETER lamb
    regulenergy=ParList[12]       # TARGET RELATIVE WEIGHT CONSTRAINT/FITNESS
    avgenergy=ParList[13]        # FIXED NUMBER OF AVERAGING CYCLES TO EVALUATE THE CONSTRAINT/FITNESS RATIO
    percenergy=ParList[14]         # MAXIMUM ACCEPTED PERCENTILE IN THE ENERGY SAMPLE SETS (integer <= 100)

    #
    # ANNEALING: WARM-UP PHASE
    Tmax=ParList[15]            # Start temperature, warm-up phase
    itwarm=ParList[16]          # Max # warm-up cycles
    incrtemp=ParList[17]          # Fixed temperature increase coefficient at warm-up
    checkwarm=ParList[18]         # Fixed milestone on warm-up cycles to check the acceptance rate
    muwarm=ParList[19]            # Minimum acceptance rate to end warm-up

    #
    # ANNEALING: SAMPLING PHASE
    itmax=ParList[20]           # Max # annealing cycles (positive integer)
    itstop=ParList[21]            # # consecutive cycles with energy variations within StopTolerance to stop annealing
    StopTolerance=ParList[22]   # Stop tolerance
    RANDPLA=2*ParList[23]        # Max planar angle perturbation at each update (floating, radians, doubled for convenience)
    RANDDIE=2*ParList[24]        # Max dihedral angle perturbation at each update (floating, radians, doubled for convenience)
    decrtemp=ParList[25]        # Fixed cooling coefficient at each annealing cicle
    ...
    LMAX=NC*DIA/np.pi     # hypothesized maximum size of a bead in the full-resolution model (nm)
    LMIN=NC**(1./3)*math.sqrt(3)*DIA # hypothesized minimum bead size in the full-resolution model (nm)
    NMAX=ParList[3]               # Maximum contact frequency: fixed or computed in the current data matrix (see function inizchain)
    extrate=ParList[4]           # Size tuning for beads at levels > 0
    ...
    chromstruct(filen,hm,conf)                             # Call recursion
...
class Chromstruct(Frame):
    ...
    def initUI(self):
        ...
        global fn, DIA, RIS, NB, NMAX, extrate, span, window,\
               ...
               RANDPLA, RANDDIE, decrtemp,path,intplot

        self.parent.title("ChromStruct 3.1")
        ...
        Label(self,text="Max contact frequency").grid(row=4,column=0)
        NMAX = Entry(self)
        NMAX.insert(0,"0")
        NMAX.grid(row=4, column=1)
        Label(self,text="Bead size ratio").grid(row=5,column=0)
        extrate = Entry(self)
        extrate.insert(0,"0.3")
        extrate.grid(row=5, column=1)
...
##        quitButton.grid(row=14,column=0)
        ...
        testo=Text(self,width=85,height=1,bg='#40E0D0')
        testo.grid(row=14,column=0,columnspan=3)
        hyperlink=HyperlinkManager(testo)
        def click():
            tkMessageBox.showinfo("Conditions of Use",warranty+conditions)
        testo.insert(INSERT,"(C) 2016, Emanuele Salerno and Claudia Caudai. License GNU-GPL v.3. ")
