...
#
class bead:
    def __init__(self,endp1=np.array([0.,0.,0.]),centrd=np.array([0.,0.,0.]),endp2=np.array([0.,0.,0.]),ext=15.):
        self.centrd=centrd
        self.endp1=endp1
        self.endp2=endp2
        self.ext=ext
...
def block(level,contacts,bigblocks,window=3,minsize=6,span=6):
    if level==0:
        blocks=[]
        bounds=[0]
        sizes=[]
...
def fitcomp(level,M,n,pairs,bi,bf,rnachip,conf,matrix,p=1):
    fithic=0.
    fitrna=0.
    fitchip=0.
...
def energy(level,pairs,bi,bf,rnachip,conf,matrix,lamb=0.,p=1):
    n=len(conf)
    M=distmatrix(conf)
    [fit,fithic,fitrna,fitchip]=fitcomp(level,M,n,pairs,bi,bf,rnachip,conf,matrix,p=1)
...
def annealing(level,pairs,bi,bf,rnachip,matrix,chain):
    n=len(matrix)
    conf=deepcopy(chain)
    conf.zerochain()
    m=len(conf)
    ...
    for j in range(avgenergy):
        for i in range(1,n):
            ...
            [fitlamb,fithic,fitrna,fitchip]=fitcomp(level,M,m,pairs,bi,bf,rnachip,conflamb,matrix)
...
#
    ...
    conf.zerochain()
##
    [Phi,fit,fithic,fitrna,fitchip,constr]=energy(level,pairs,bi,bf,rnachip,conf,matrix,lamb) # Initial energy
    ...
    while iterat < itwarm:
        ...
        for i in range(1,n):
            ...
            [Phistar,fitstar,fithic,fitrna,fitchip,constrstar]=energy(level,pairs,bi,bf,rnachip,confstar,matrix,lamb)
...
##
    ...
    while not checkstop(Phiseries,iterat): 
        ...
        for i in range(1,n):
            ...
            [Phistar,fit,fithic,fitrna,fitchip,constr]=energy(level,pairs,bi,bf,rnachip,confstar,matrix,lamb,1)
            if checktest(Phi,Phistar,T):
                naccept=naccept+1
                Taccept.append(T)
...
#
# Sets the regularization parameter, sets the start temperature, samples the energy distribution
def diffannealing(level,pairs,bi,bf,rnachip,matrix,chain):
    T=900000
    decrtemp=0.9
    n=len(matrix)
    conf=deepcopy(chain)
...
def savepar(filen,EHist,level,blockn):
...
# FUNCTION saveconf
#
# Saves a chain configuration to file (txt and/or pdb format)
def saveconf(filen,conf,level,blockn):
    if level==-1:  # level is set to -1 when the final configuration is to be saved
        print('Chain length', len(conf))
        M=distmatrix(conf)
        np.savetxt(filen+'_'+timemark+'_DistMat.txt',M) # Save the distance matrix
        if (format1==0 and format2==0) or format1==1:
            out=open(filen+'_'+timemark+'_LastConf.txt',"w")
            out.write(str(conf))
            out.close()
        if format2==1:
            out=open(filen+'_'+timemark+'_LastConf.pdb',"w")
            for i in range(1,len(conf)+1):
                string0=[]
                if i<10:
                    string0 = "ATOM      "+str(i)+" C"
                elif i<100:
                    string0 = "ATOM     "+str(i)+" C"
                elif i<1000:
                    string0 = "ATOM    "+str(i)+" C"
                    ...
                    string0 = "ATOM  "+str(i)+" C"
                string1=[]
                coord=conf.bead[i-1].centrd.x/100
                if coord>=0:
                    if coord<10:
                        string1 = " "+"%.3f" % coord
                    elif coord<100:
                    ...
                    if abs(coord)<10:
                        ...
                        string1 = "%.0f" % coord
                string2=[]
                coord=conf.bead[i-1].centrd.y/100
                if coord>=0:
                    if coord<10:
                        string2 = " "+"%.3f" % coord
                    elif coord<100:
                    ...
                    if abs(coord)<10:
                        ...
                        string2 = "%.0f" % coord
                string3=[]
                coord=conf.bead[i-1].centrd.z/100
                if coord>=0:
                    if coord<10:
                        string3 = " "+"%.3f" % coord
                    elif coord<100:
                    ...
                    if abs(coord)<10:
                        ...
                        string3 = "%.0f" % coord
                string = string0+"    LIG A         "+string1+"   "+string2+"   "+string3+"    \n"
                out.write(string)           
            for i in range(2,len(conf)+1):
                string0=[]
                if i<10:
                    string0 = "CONECT    "+str(i-1)+"    "+str(i)+"\n"
                elif i==10:
                    string0 = "CONECT    "+str(i-1)+"   "+str(i)+"\n"
                elif i<100:
                    string0 = "CONECT   "+str(i-1)+"   "+str(i)+"\n"
        ...
        if (format1==0 and format2==0) or format1==1:
            ...
            out.close()
        if format2==1:
            out=open(filen+'_'+timemark+'_'+str(level)+'_'+str(blockn)+'.pdb',"w")
            for i in range(1,len(conf)+1):
                string0=[]
                if i<10:
                    string0 = "ATOM      "+str(i)+" C"
...
def chromstruct(filen,contacts,chaininf,bigblocks,rnachip,level=0):
    nblocks=[len(contacts)]
    nlevel=0
    while nblocks[nlevel]>1:
        [blocks,sizes,bounds]=block(nlevel,contacts,bigblocks,minsize=minsize,window=window,span=span) # extracts the TADs at the current level
...
##                print(chainlist[i])
            ...
            if blocks[i].max()==-1:
                ...
                [C, Tseries, Phiseries, iteraccept] = diffannealing(nlevel,L,bounds[i],bounds[i+1]-1,rnachip,blocks[i],chainlist[i]) # Sample the current subchain
            else:
                print('bi=',bounds[i],'bf=',bounds[i+1])
                [C, Tseries, Phiseries, iteraccept] = annealing(nlevel,L,bounds[i],bounds[i+1]-1,rnachip,blocks[i],chainlist[i]) # Sample the current subchain
...
##                pylab.show()
            ...
            savepar(filen,Phiseries, nlevel, i)                  # Save the features of the annealing schedule
            saveconf(filen,C,nlevel,i)                           # Save current subchain configuration
...
def mainchrom(filen,chip,rna,ctcf,ParList):
    global diagneg, datafact,energy_scale, energy_exp, span, window, minsize, regulenergy,\
           avgenergy, percenergy, Tmax, itwarm, incrtemp, checkwarm, muwarm, itmax, itstop,\
           StopTolerance, RANDPLA, RANDDIE, decrtemp, DIA, RIS, NB, crate, NC, LMAX, LMIN, NMAX, extrate,\
           timemark, outdata, display, format1, format2
    ...
    #
    # Fitness part
    diagneg=ParList[9]             # # neglected diagonals in contact matrix (in populate, to select the relevant pairs for data fit)
    datafact=ParList[10]           # Fraction of the pairs per block used to build the penalty function (real, in populate)

    # Constraint part
    energy_scale=ParList[11]    # Tunes the extent of the moderate penalty range around the threshold distance 
                               # (floating; physical dimensions 1/length; measurement unit: reciprocal of the units used for DIA)
    energy_exp=ParList[12]               # Tunes the slopes of the penalty in the transitions between the moderate range
                               # and the external intervals (odd integer, dimensionless)

    #
    # TAD EXTRACTION (FUNCTION block)
    span=ParList[6]                # SIZE OF THE MOVING TRIANGLE
    window=ParList[7]              # MOVING AVERAGE WINDOW APERTURE
    minsize=ParList[8]             # MINIMUM REQUIRED DIAGONAL BLOCK SIZE


    #
    # ANNEALING: DETERMINATION OF THE REGULARIZATION PARAMETER lamb
    regulenergy=ParList[13]       # TARGET RELATIVE WEIGHT CONSTRAINT/FITNESS
    avgenergy=ParList[14]        # FIXED NUMBER OF AVERAGING CYCLES TO EVALUATE THE CONSTRAINT/FITNESS RATIO
    percenergy=ParList[15]         # MAXIMUM ACCEPTED PERCENTILE IN THE ENERGY SAMPLE SETS (integer <= 100)

    #
    # ANNEALING: WARM-UP PHASE
    Tmax=ParList[16]            # Start temperature, warm-up phase
    itwarm=ParList[17]          # Max # warm-up cycles
    incrtemp=ParList[18]          # Fixed temperature increase coefficient at warm-up
    checkwarm=ParList[19]         # Fixed milestone on warm-up cycles to check the acceptance rate
    muwarm=ParList[20]            # Minimum acceptance rate to end warm-up

    #
    # ANNEALING: SAMPLING PHASE
    itmax=ParList[21]           # Max # annealing cycles (positive integer)
    itstop=ParList[22]            # # consecutive cycles with energy variations within StopTolerance to stop annealing
    StopTolerance=ParList[23]   # Stop tolerance
    RANDPLA=2*ParList[24]        # Max planar angle perturbation at each update (floating, radians, doubled for convenience)
    RANDDIE=2*ParList[25]        # Max dihedral angle perturbation at each update (floating, radians, doubled for convenience)
    decrtemp=ParList[26]        # Fixed cooling coefficient at each annealing cicle


    #
    # GEOMETRIC PARAMETERS
    DIA=ParList[0]              # Chromatin fiber diameter (nm)
    RIS=ParList[1]              # Contact matrix (full) genomic resolution (kbp)
    NB=ParList[2]                 # kilobase-pairs in a chromatin fragment with length DIA
    crate=ParList[3]            # compactness rate for telomeres and centromeres

        
    NC=RIS/NB             # # fragments per genomic resolution unit
    LMAX=NC*DIA/np.pi     # hypothesized maximum size of a bead in the full-resolution model (nm)
    LMIN=NC**(1./3)*math.sqrt(3)*DIA # hypothesized minimum bead size in the full-resolution model (nm)
    NMAX=ParList[4]               # Maximum contact frequency: fixed or computed in the current data matrix (see function inizchain)
    extrate=ParList[5]           # Size tuning for beads at levels > 0

    #
    # OUTPUT FORMAT
    format1=ParList[28]  # optional txt format of output files
    format2=ParList[29]  # optional PDB format of output files
    ...
    try: ch=np.loadtxt(chip)              # Read ChIPseq data
    except:
        print('No such file: no data read\n')

    try: rn=np.loadtxt(rna)              # Read RNAseq data
    except:
        print('No such file: no data read\n')
        
    try: ctcf=np.loadtxt(ctcf)              # Read CTCF data
    except:
        print('No such file: no data read\n')
    ...
    for i in range(len(hm)):                         # Set the matrix lower triangle to zero
        for j in range(i):
            hm[i,j]=0.
    ...
    chromstruct(filen,hm,conf,bigblocks,rnachip,)                             # Call iteration
...
class Chromstruct(Frame):
    ...
    def initUI(self):
        ...
        global fn, DIA, RIS, NB, crate, NMAX, extrate, span, window,\
               minsize, diagneg, datafact,scale, exponent, \
               regulenergy, avgenergy, percenergy, Tmax, itwarm,\
               incrtemp, checkwarm, muwarm, itmax, itstop, stoptol,\
               RANDPLA, RANDDIE, decrtemp,path,path1, path2, path3, intplot, format1, format2

        self.parent.title("ChromStruct 4.3")
        ...
        self.rowconfigure(22, pad=3)
        

        Label(self,text="GEOMETRY").grid(row=0,column=1)
        Label(self,text="METHOD").grid(row=0,column=3)
        Label(self,text="ALGORITHM").grid(row=0,column=5)
        Label(self,text="Diameter (nm)",anchor=W,justify=LEFT).grid(row=1,column=0)

        DIA = Entry(self)
        DIA.insert(0,"30")
        DIA.grid(row=1, column=1)
        Label(self,text="Original resolution (kbp)").grid(row=2,column=0)
        RIS = Entry(self)
        RIS.insert(0,"100")
        RIS.grid(row=2, column=1)
        Label(self,text="kbp in the smallest bead").grid(row=3,column=0)
        NB = Entry(self)
        NB.insert(0,"3.")
        NB.grid(row=3, column=1)
        Label(self,text="centromeres and telomeres dim").grid(row=4,column=0)
        crate = Entry(self)
        crate.insert(0,"0.002")
        crate.grid(row=4, column=1)        
        Label(self,text="Max contact frequency").grid(row=5,column=0)
        NMAX = Entry(self)
        NMAX.insert(0,"0")
        NMAX.grid(row=5, column=1)
        Label(self,text="Bead size ratio").grid(row=6,column=0)
        extrate = Entry(self)
        extrate.insert(0,"0.3")
        extrate.grid(row=6, column=1)
        Label(self,text="Blocks").grid(row=1,column=3,columnspan=1)
        Label(self,text="Moving avg window").grid(row=2,column=2)
        span = Entry(self)
        span.insert(0,"7")
        span.grid(row=2, column=3)
        Label(self,text="Smoothing window").grid(row=3,column=2)
        window = Entry(self)
        window.insert(0,"3")
        window.grid(row=3, column=3)
        Label(self,text="Min block size").grid(row=4,column=2)
        minsize = Entry(self)
        minsize.insert(0,"7")
        minsize.grid(row=4, column=3)
        Label(self,text="Score").grid(row=5,column=3,columnspan=1)
        Label(self,text="Data: No. of neglected diagonals").grid(row=6,column=2)
        diagneg = Entry(self)
        diagneg.insert(0,"2")
        diagneg.grid(row=6, column=3)
        Label(self,text="Data: Relevant pairs ratio").grid(row=7,column=2)
        datafact = Entry(self)
        datafact.insert(0,"0.3")
        datafact.grid(row=7, column=3)
        Label(self,text="Constraint: scale").grid(row=8,column=2)
        scale = Entry(self)
        scale.insert(0,"4.")
        scale.grid(row=8, column=3)
        Label(self,text="Constraint: exponent").grid(row=9,column=2)
        exponent = Entry(self)
        exponent.insert(0,"5")
        exponent.grid(row=9, column=3)
        Label(self,text="Constraint/data balance").grid(row=1,column=4)
        regulenergy = Entry(self)
        regulenergy.insert(0,"0.005")
        regulenergy.grid(row=1, column=5)
        Label(self,text="Balance sampling cycles").grid(row=2,column=4)
        avgenergy = Entry(self)
        avgenergy.insert(0,"500")
        avgenergy.grid(row=2, column=5)
        Label(self,text="Balance - averaging percentile").grid(row=3,column=4)
        percenergy = Entry(self)
        percenergy.insert(0,"20")
        percenergy.grid(row=3, column=5)
        Label(self,text="Guessed start temperature").grid(row=4,column=4)
        Tmax = Entry(self)
        Tmax.insert(0,"4000")
        Tmax.grid(row=4, column=5)
        Label(self,text="Max warming cycles").grid(row=5,column=4)
        itwarm = Entry(self)
        itwarm.insert(0,"50000")
        itwarm.grid(row=5, column=5)
        Label(self,text="Warm-up rate").grid(row=6,column=4)
        incrtemp = Entry(self)
        incrtemp.insert(0,"1.2")
        incrtemp.grid(row=6, column=5)
        Label(self,text="Warm-up checking period").grid(row=7,column=4)
        checkwarm = Entry(self)
        checkwarm.insert(0,"500")
        checkwarm.grid(row=7, column=5)
        Label(self,text="Min acceptance rate to start cooling").grid(row=8,column=4)
        muwarm = Entry(self)
        muwarm.insert(0,"0.9")
        muwarm.grid(row=8, column=5)
        Label(self,text="Max annealing cycles").grid(row=9,column=4)
        itmax = Entry(self)
        itmax.insert(0,"50000")
        itmax.grid(row=9, column=5)
        Label(self,text="No. of cycles for tolerance check").grid(row=10,column=4)
        itstop = Entry(self)
        itstop.insert(0,"500")
        itstop.grid(row=10, column=5)
        Label(self,text="Stop tolerance").grid(row=11,column=4)
        stoptol = Entry(self)
        stoptol.insert(0,"1.e-2")
        stoptol.grid(row=11, column=5)
        Label(self,text="Planar angles step").grid(row=12,column=4)
        RANDPLA = Entry(self)
        RANDPLA.insert(0,"0.05")
        RANDPLA.grid(row=12, column=5)
        Label(self,text="Dihedral angles step").grid(row=13,column=4)
        RANDDIE = Entry(self)
        RANDDIE.insert(0,"0.05")
        RANDDIE.grid(row=13, column=5)
        Label(self,text="Cool-down rate").grid(row=14,column=4)
        decrtemp = Entry(self)
        decrtemp.insert(0,"0.998")
        decrtemp.grid(row=14, column=5)
        start = Button(self, text="START",command=self.startchrom)
        start.grid(row=9, column=0,columnspan=2)
        fn=StringVar()
        
        filen = Button(self, text="HiC INPUT DATA",command=self.openfile)
        filen.grid(row=11, column=0,columnspan=2)
        
        Label(self,text="File").grid(row=12,column=0)
        path=Entry(self,width=70)
        path.grid(row=12,column=1,columnspan=3)
        
        chip = Button(self, text="ChIPseq H3K27me3",command=self.openfile1)
        chip.grid(row=13, column=0,columnspan=2)
        
        Label(self,text="File").grid(row=14,column=0)
        path1=Entry(self,width=70)
        path1.grid(row=14,column=1,columnspan=3)
        
        rna = Button(self, text="RNAseq Active Genes",command=self.openfile2)
        rna.grid(row=15, column=0,columnspan=2)
        
        Label(self,text="File").grid(row=16,column=0)
        path2=Entry(self,width=70)
        path2.grid(row=16,column=1,columnspan=3)
        
        ctcf = Button(self, text="CTCF Binding Sites",command=self.openfile3)
        ctcf.grid(row=17, column=0,columnspan=2)
        
        Label(self,text="File").grid(row=18,column=0)
        path3=Entry(self,width=70)
        path3.grid(row=18,column=1,columnspan=3)

        intplot=IntVar()
        c=Checkbutton(self,text="Display intermediate plots",variable=intplot)
        c.grid(row=10,column=1)

        Label(self,text="Output format").grid(row=20,column=0,columnspan=2)
        format1=IntVar()
        Checkbutton(self,text="txt",variable=format1).grid(row=20,column=0, columnspan=3)
        format2=IntVar()
        Checkbutton(self,text="PDB",variable=format2).grid(row=20,column=0, columnspan=4)
        
##        quitButton=Button(self,text="Quit",command=self.quit)
##        quitButton.grid(row=14,column=0)

        testo=Text(self,width=91,height=1,bg='#40E0D0')
        testo.grid(row=22,column=0,columnspan=3)
        hyperlink=HyperlinkManager(testo)
        def click():
            messagebox.showinfo("Conditions of Use",warranty+conditions)
        testo.insert(INSERT,"(C) 2020, Emanuele Salerno and Claudia Caudai. License GNU-GPL v.3. ")
        testo.insert(INSERT,"Conditions of use",hyperlink.add(click))
    ...
    def openfile(self,*args):
        global fn, path
        fn.set(filedialog.askopenfilename())
        path.insert(0,fn.get())
        return fn
    
    def openfile1(self,*args):
        global fn, path1
        fn.set(filedialog.askopenfilename())
        path1.insert(0,fn.get())
        return fn
    
    def openfile2(self,*args):
        global fn, path2
        fn.set(filedialog.askopenfilename())
        path2.insert(0,fn.get())
        return fn
    
    def openfile3(self,*args):
        global fn, path3
        fn.set(filedialog.askopenfilename())
        path3.insert(0,fn.get())
        return fn
    
    def startchrom(self,*args):
        global fn, DIA, RIS, NB, crate, NMAX, extrate, span, window,\
               minsize, diagneg, datafact,scale, exponent, \
               regulenergy, avgenergy, percenergy, Tmax, itwarm,\
               incrtemp, checkwarm, muwarm, itmax, itstop, stoptol,\
               RANDPLA, RANDDIE, decrtemp,path,path1,path2,path3,intplot, format1, format2
        #
        # BUILDING THE LIST OF PARAMETERS TO PASS TO ChromStruct
        #
        ParList=[]
        ParList.append(float(DIA.get()))
        ParList.append(float(RIS.get()))
        ParList.append(float(NB.get()))
        ParList.append(float(crate.get()))
        ParList.append(float(NMAX.get()))
        ParList.append(float(extrate.get()))
        ParList.append(int(span.get()))
        ParList.append(int(window.get()))
        ParList.append(int(minsize.get()))
        ParList.append(int(diagneg.get()))
        ParList.append(float(datafact.get()))
        ParList.append(float(scale.get()))
        ParList.append(int(exponent.get()))
        ParList.append(float(regulenergy.get()))
        ParList.append(int(avgenergy.get()))
        ParList.append(int(percenergy.get()))
        ParList.append(float(Tmax.get()))
        ParList.append(int(itwarm.get()))
        ParList.append(float(incrtemp.get()))
        ParList.append(int(checkwarm.get()))
        ParList.append(float(muwarm.get()))
        ParList.append(int(itmax.get()))
        ParList.append(int(itstop.get()))
        ParList.append(float(stoptol.get()))
        ParList.append(float(RANDPLA.get()))
        ParList.append(float(RANDDIE.get()))
        ParList.append(float(decrtemp.get()))
        ParList.append(intplot.get())
        ParList.append(format1.get())
        ParList.append(format2.get())
        #
        # CALLING ChromStruct
        #
        filen=path.get()
        ...
        rna=path2.get()
        ctcf=path3.get()
        mainchrom(filen,chip,rna,ctcf,ParList)
